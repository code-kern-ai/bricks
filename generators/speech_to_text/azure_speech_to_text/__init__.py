...
def azure_speech_to_text(req: AzureSpeechToTextModel):
    ...
    response = requests.post(
        "https://" + req.region + ".stt.speech.microsoft.com/speech/recognition/conversation/cognitiveservices/v1",
        params=params,
        headers=headers,
        data=wav.content,
    )
    ...
    if response.json()["Duration"] == 0:
        return {"transcription": "The transcription was not successful. Did you provide a valid link to a .wav file?."}
    else:
        try:
            return {"transcription": response.json()["DisplayText"]}
        except:
            return {"transcription": "No text to display. Did you provide a valid link to a .wav file?."}